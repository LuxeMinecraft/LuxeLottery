...
public class LotteryCommand implements CommandExecutor {
    ...
    public void parseCommand(Player player, String[] split) {
        if (split.length == 0) {
            Deity.chat.sendPlayerMessage(player, "LuxeLottery", "&8There is &e" + LotteryObject.getPot() + ".00" + " &8Dei in the current pot.");
            Deity.chat.sendPlayerMessage(player, "LuxeLottery", "&8You have &e" + LotteryObject.getNumTickets(player.getName()) + " &8Tickets.");
            Deity.chat.sendPlayerMessage(player, "LuxeLottery", "&8Tickets are &e" + Lottery.config.getInt("lottery.ticket_price") + " &8each.");
            Deity.chat.sendPlayerMessage(player, "LuxeLottery", "&eUse /lottery ? for a list of commands.");
    ...
    public void buyCommand(Player player, String[] split) throws InvalidFundsException, NegativeMoneyException, InvalidChannelException {
        ...
        if (Deity.econ.canPay(player.getName(), money)) {
            ...
            if (numTicket == 1) Deity.chat.sendMessageToAllOnline("LuxeLottery", "&e" + player.getName() + " &fjust bought &e1 ticket! &3/lottery buy&f to get one yourself");
            else Deity.chat.sendMessageToAllOnline("LuxeLottery", "&e" + player.getName() + " &fjust bought &e" + numTicket + " tickets! &3/lottery buy&f to get one yourself");
        } else {
